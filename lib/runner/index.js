...
module.exports = class TestsRunner extends Runner {
    ...
    run(suiteCollection) {
        return Promise.resolve(this.emitAndWait(Events.START_RUNNER, this))
            ...
            .finally(() => {
                this.emit(Events.END, this._counter.getResult());
    ...
    _initTestCounter(runner) {
        runner.on(Events.SKIP_STATE, (result) => this._counter.onSkipped(result));
        runner.on(Events.ERROR, (result) => this._counter.onFailed(result));
        runner.on(Events.UPDATE_RESULT, (result) => {
            result.updated ? this._counter.onUpdated(result) : this._counter.onPassed(result);
        });
        runner.on(Events.TEST_RESULT, (result) => {
            result.equal ? this._counter.onPassed(result) : this._counter.onFailed(result);
        });
        runner.on(Events.RETRY, (result) => this._counter.onRetry(result));
    }
